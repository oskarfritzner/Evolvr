...
export default function Dashboard() {
  ...
  useEffect(() => {
    const checkFirstVisit = async () => {
      try {
        ...
        const hasSeenWelcome = await AsyncStorage.getItem(`welcome_seen_${userData.userId}`);
        if (!hasSeenWelcome && userData.onboardingComplete) {
          setShowWelcomeModal(true);
        }
      } catch (error) {
        logger.error('Error checking first visit:', error);
      }
    };
  ...
  const handleWelcomeModalClose = async () => {
    try {
      if (userData?.userId) {
        await AsyncStorage.setItem(`welcome_seen_${userData.userId}`, 'true');
      }
      setShowWelcomeModal(false);
    } catch (error) {
      logger.error('Error saving welcome modal state:', error);
    }
  };
