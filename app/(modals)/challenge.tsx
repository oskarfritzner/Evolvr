...
export default function ChallengeModal() {
  ...
  useEffect(() => {
    if (challenge?.id && userChallenges) {
      const isUserParticipating = userChallenges.some(c => c.id === challenge.id);
      setIsParticipating(isUserParticipating);
    }
  }, [challenge?.id, userChallenges]);

  useEffect(() => {
    console.log("User challenges:", user?.userData?.challenges);
    console.log("Current challenge ID:", challenge?.id);
    if ((user?.userData?.challenges?.length ?? 0) > 0) {
      console.log("First challenge in user data:", user?.userData?.challenges[0]);
      console.log("ID comparison:", {
        challengeId: challenge?.id,
        userChallengeId: user?.userData?.challenges[0]?.id,
        areEqual: user?.userData?.challenges[0]?.id === challenge?.id
      });
    }
    console.log("Is user in challenge:", user?.userData?.challenges?.some(c => c.id === challenge?.id));
  }, [user?.userData?.challenges, challenge]);
  ...
  const handleCompleteChallenge = async (challengeId: string) => {
    if (!user?.uid || !challenge?.id) return;
    try {
      await challengeService.completeChallenge(user.uid, challenge.id);
      Toast.show({
        type: 'success',
