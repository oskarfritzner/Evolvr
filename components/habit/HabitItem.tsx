...
export default function HabitItem({ habit, onRefresh, onDelete }: HabitItemProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = habitService.onHabitUpdated(habit.userId, habit.id, () => {
      if (onRefresh) onRefresh();
    });
    return () => unsubscribe();
  }, [habit?.id, habit?.userId, onRefresh]);

  const progressDays = Array(66).fill(null).map((_, index) => {
    return habit.completedDays[index] || {
      date: new Date(),
      completed: false
    };
  });
            ...
            <View style={styles.completionGrid}>
              {progressDays.map((day, index) => (
                <View 
                  key={index}
                  style={[styles.dayBox, {
                    backgroundColor: day.completed ? colors.secondary : 'transparent',
                    borderColor: colors.border,
                  }]}
                >
                  <Text style={[styles.dayText, { 
                    color: day.completed ? colors.surface : colors.textSecondary,
