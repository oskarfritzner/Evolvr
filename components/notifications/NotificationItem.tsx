...
import { ChallengeInviteNotification, FriendRequestNotification, Notification as NotificationType, NotificationType as NotificationTypes, RoutineInviteNotification } from '@/backend/types/Notification';
...
export default function Notification({ 
  ...
  const hasResponseActions = (notification: NotificationType) => {
    return notification.type === NotificationTypes.FRIEND_REQUEST || 
           notification.type === NotificationTypes.ROUTINE_INVITE ||
           notification.type === NotificationTypes.CHALLENGE_INVITE;
  };
  ...
  const renderActions = () => {
    if (!isRespondableNotification(notification)) {
    ...
    if (notification.type === NotificationTypes.ROUTINE_INVITE) {
    ...
    if (notification.type === NotificationTypes.ROUTINE_DELETED) {
      if (!notification.responded) {
        return (
          <View style={styles.actions}>
            <TouchableOpacity
              ...
              onPress={onAccept}
            >
              <Text style={styles.actionButtonText}>Continue Solo</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.actionButton, styles.declineButton, { borderColor: colors.textSecondary }]}
              onPress={onDecline}
            >
    ...
    if (!hasResponseActions(notification)) return null;
    if ('responded' in notification && notification.responded) return null;
