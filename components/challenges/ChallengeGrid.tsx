...
export default function ChallengeGrid({ style, compact = false }: Props) {
  ...
  const { data: activeUserChallenges = [] } = useQuery({
    queryKey: ['userChallenges', user?.uid],
    queryFn: async () => {
      if (!user?.uid) return [] as UserChallenge[];
      const userData = await userService.getUserData(user.uid);
      return (userData?.challenges || []) as UserChallenge[];
    },
    enabled: !!user?.uid,
  });
  ...
  const checkFailedChallenges = async () => {
    ...
    if (failedChallenges.length > 0) {
      setFailedChallenge(failedChallenges[0]); // Show first failed challenge
    }
  };
    ...
    <View style={[styles.container, style]}>
      ...
      {activeUserChallenges.length === 0 ? (
        ...
        <View style={styles.grid}>
          {activeUserChallenges.map((challenge: UserChallenge) => (
            <ChallengeCard 
              key={challenge.id}
              challenge={challenge}
