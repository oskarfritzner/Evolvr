...
export const routineService = {
  ...
  async saveRoutine(
    ...
    try {
      ...
      // Get creator's user data for the notification
      const creatorDoc = await getDoc(userRef);
      const creatorData = creatorDoc.data();
      const creatorName =
        creatorData?.displayName || creatorData?.username || "A user";

      // Create new routine with explicit structure
      const newRoutine = {
        id: routineId,
        title: routineData.title || "",
        description: routineData.description || "",
        tasks: routineData.tasks || [],
        invites: routineData.invites || [], // Array of invited user IDs
        participants: [userId], // Starts with creator, grows as people accept
        createdBy: userId,
        createdAt: Timestamp.now(),
        active: true,
        xpGains: routineData.xpGains || {},
        metadata: {
          currentStreak: 0,
          bestStreak: 0,
          lastCompleted: null,
      ...
      // Set in routines collection
      batch.set(routineRef, newRoutine);

      // Send notifications to invited users
      if (routineData.invites && routineData.invites.length > 0) {
        console.log("Sending routine invites to:", routineData.invites);

        for (const invitedUserId of routineData.invites) {
          const invitedUserRef = doc(db, "users", invitedUserId);

          // Create routine invite notification
          batch.update(invitedUserRef, {
            [`notifications.${routineId}`]: {
              id: routineId,
              type: NotificationType.ROUTINE_INVITE,
              routineId,
              routineName: newRoutine.title,
              inviterId: userId,
              inviterName: creatorName,
              inviterPhotoURL: creatorData?.photoURL,
              routine: {
                title: newRoutine.title,
                description: newRoutine.description || "",
                tasks: newRoutine.tasks.map((task) => ({
                  id: task.id,
                  title: task.title,
                  days: task.days,
      ...
      // Update creator's cached routines
      batch.update(userRef, {
        lastRoutineSync: null, // Force fresh fetch
        [`routines.${routineId}`]: null, // Clear cached routine data
      });

      await batch.commit();
      console.log("Successfully created routine with invites");
    } catch (error) {
  ...
  async deleteRoutine(userId: string, routineId: string): Promise<boolean> {
    try {
      console.log("Attempting to delete routine:", {
        routineId,
        userId,
        collection: "routines",
      });
      ...
      console.log("Routine data:", {
        ...
        userId,
        isCreator: routine.createdBy === userId,
      });
      ...
      // Try each operation separately to identify which one fails
      try {
        // 1. First try updating participants
        for (const participantId of routine.participants.filter(
          (id) => id !== userId
        )) {
        ...
        // 3. Finally try deleting the routine
        console.log("Deleting routine document");
  ...
  async leaveRoutine(
    ...
    try {
      console.log("Attempting to leave routine:", {
        routineId,
        userId,
        keepPersonal,
      });
      ...
      await batch.commit();
      console.log("Successfully left routine");
    } catch (error) {
  ...
  async completeRoutineTask(
    ...
    try {
      console.log("Starting completeRoutineTask:", {
        userId,
        taskId,
        routineId,
      });
      ...
      // Update only the specific task's completions using arrayUnion
      // This preserves the array structure and only updates the completions
        ...
        [`tasks`]: tasksArray.map((task, index) =>
          index === taskIndex
            ? {
                ...
                completions: {
                  ...task.completions,
                  [today]: [...(task.completions?.[today] || []), completion],
                },
              }
            : task
        ),
  ...
  async getActiveRoutineTasks(userId: string): Promise<RoutineTaskWithMeta[]> {
    try {
      ...
      for (const routine of routines) {
        console.log("Processing routine:", routine.id, routine.tasks);
        ...
        for (const task of tasksArray) {
          ...
          if (task.active === false) {
            console.log("Skipping inactive task:", task.id);
            continue;
          }
          ...
          // Only filter out tasks if ALL participants have completed them
          if (!allCompleted) {
            routineTasks.push({
              id: task.id,
              taskId: task.id,
      ...
      console.error("Error getting today's routine tasks:", error);
      return [];
    }
  },
  ...
  async addParticipant(routineId: string, userId: string): Promise<void> {
    try {
      console.log("Adding participant to routine:", {
        routineId,
        userId,
      });
  ...
  async removeInvite(routineId: string, userId: string): Promise<void> {
    try {
      console.log("Removing invite from routine:", {
        routineId,
        userId,
      });
