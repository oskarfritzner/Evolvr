...
export const levelService = {
  ...
  getCategoryLevelProgress(currentXP: number, level: number): number {
    ...
    console.log("Level Progress Calculation:", {
      ...
      xpInCurrentLevel,
      xpNeededForLevel,
      progress: xpInCurrentLevel / xpNeededForLevel,
    });
  ...
  async handlePrestige(userId: string): Promise<boolean> {
    try {
      ...
      if (!userData.overall || userData.overall.level < MAX_LEVEL) {
        console.log("User does not meet prestige requirements", {
          currentLevel: userData.overall?.level,
  ...
  async updateUserLevels(
    ...
    try {
      logger.debug("[LevelService] Updating levels for user:", userId);
      logger.debug("[LevelService] XP to add:", categoryXp);
      ...
      logger.debug("[LevelService] Current user data:", userData);
      ...
      logger.debug("[LevelService] XP update complete:", userData.categories);
    } catch (error) {
      logger.error("[LevelService] Error updating levels:", error);
      throw error;
    }
  },
  ...
  async addXP(
    ...
    try {
      console.log("1. Initial XP gains:", { xpGains, taskType });
      ...
      console.log("2. Current daily XP:", todayXP);

      const baseXPTotal = Object.values(xpGains).reduce(
        (sum, xp) => sum + (xp || 0),
        0
      );
      console.log("3. Base XP total:", baseXPTotal);

      let totalXPToAdd = baseXPTotal;
      let xpMultiplier = 1;
      if (taskType !== "normal") {
        xpMultiplier = this.calculateXPMultiplier(userData, taskType, userId);
        totalXPToAdd = Math.floor(baseXPTotal * xpMultiplier);
        console.log("4. Applied multiplier:", { xpMultiplier, totalXPToAdd });
      }
      ...
      console.log("5. Adjusted XP:", { remainingDailyXP, adjustedXPToAdd });

      const adjustmentFactor =
        taskType === "normal" ? 1 : adjustedXPToAdd / totalXPToAdd;
      console.log("6. Adjustment factor:", adjustmentFactor);
      ...
      console.log("7. Final adjusted XP gains:", adjustedXpGains);
      ...
      for (const [category, xp] of Object.entries(adjustedXpGains)) {
        ...
        if (newLevel > currentLevel) {
          ...
          // Level up notification is now handled separately
        }
      }
      ...
      // Check for new badges after updating XP and levels
      let newBadges: string[] = [];
      try {
        const badgeParams: BadgeCheckParams = {
          taskType,
          categories: Object.keys(xpGains),
          xpGained: Object.values(xpGains).reduce((sum, xp) => sum + xp, 0),
        };
        newBadges = await badgeService.checkAndAwardBadges(userId, badgeParams);
      } catch (error) {
